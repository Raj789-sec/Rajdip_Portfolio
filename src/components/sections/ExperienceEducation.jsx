    ...
    <section id="experience" className="relative py-20 px-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-4">
          {jobs.map((job, i) => (
            <motion.div key={i} {...fade(0.1 * i)} className={`bento ${i === 0 ? "bento-accent" : ""} p-7`}>
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-violet-400/70 font-medium mt-0.5">{job.company}</p>
                </div>
                <div className="flex items-center gap-4 text-xs text-white/25 shrink-0">
              ...
              <p className="mt-3 text-sm text-white/35 leading-relaxed">{job.description}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {job.tags.map((t) => (
                  <span key={t} className="rounded-lg bg-white/[0.03] border border-white/[0.06] px-2.5 py-1 text-[11px] text-white/30">
                    {t}
                  </span>
                ))}
              </div>
