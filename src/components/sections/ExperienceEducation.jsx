    ...
    <section id="experience" className="relative py-14 px-3 sm:py-20 sm:px-6 md:px-8">
      <div className="mx-auto w-full max-w-6xl">
        ...
        <div className="text-center mb-8 sm:mb-10">
          ...
          <div className="mt-5 mx-auto w-40 sm:w-64 h-[5px] heading-track rounded-full overflow-hidden">
            <span className="heading-runner" />
          </div>
        </div>

        {/* OUTER SHELL */}
        <div className="rounded-2xl sm:rounded-3xl p-4 sm:p-10 section-shell">
...
function SubHeading({ icon, text }) {
  return (
    <div className="mb-5 sm:mb-6">
      ...
      <div className="mt-3 h-[3px] w-32 sm:w-40 heading-track rounded-full overflow-hidden">
        <span className="heading-runner" />
      </div>
    </div>
  );
...
function Card({ children }) {
  return (
    <motion.div
      ...
      className="rounded-2xl border border-white/10 bg-[#0b1220]/90 backdrop-blur-sm shadow-[0_8px_24px_rgba(0,0,0,0.45)] p-4 sm:p-6 relative overflow-hidden"
    >
...
function Header({ title, right }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
      ...
      {right && (
        <div className="text-xs sm:text-sm text-white/60 inline-flex items-center gap-1.5 sm:gap-2">
...
function Meta({ location, extras = [] }) {
  ...
  return (
    <div className="mt-1 text-white/70 text-xs sm:text-sm flex flex-wrap items-center gap-x-3 gap-y-1.5">
      {location && (
      ...
      {extras.map((e, i) => (
        <span key={i} className="inline-flex items-center gap-1.5 text-white/70">
          {e.icon} {e.text}
        </span>
      ))}
    </div>
...
function Description({ text }) {
  return <p className="mt-3 sm:mt-4 text-white/80 text-sm sm:text-base leading-relaxed">{text}</p>;
}
...
function StyleBlock() {
  return (
    <style>{`
      ...
      .heading-runner{ display:block; height:100%; width:34%; background:linear-gradient(90deg,#22d3ee,#3b82f6); animation:dash 3s linear infinite; }
      ...
      @media (min-width: 640px){
        .tech-badge{ font-size:16px; padding:10px 16px; }
      }
      ...
      /* on hover/focus: fill + glow + slight motion (desktop pointers) */
      ...
      @media (prefers-reduced-motion: reduce){
        .glitch-title,.glitch-title::before,.glitch-title::after,
        .glitch-sub::before,.glitch-sub::after,
        .heading-runner,.glitch-gradient{ animation: none !important; }
      }
