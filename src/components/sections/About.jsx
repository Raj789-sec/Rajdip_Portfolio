...
export default function About() {
  return (
    <section id="about" className="relative py-24 px-6 bg-surface-100">
      <div className="mx-auto max-w-6xl">
        <motion.div {...fade()}>
          <p className="section-label">About</p>
          <h2 className="text-3xl sm:text-4xl font-heading font-extrabold tracking-tight mb-12">
            Who I am
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          <motion.div {...fade(0.1)} className="md:col-span-7 card-accent p-8">
            <p className="text-navy-600 leading-relaxed font-body text-[15px]">{SUMMARY}</p>
          ...
          <motion.div {...fade(0.2)} className="md:col-span-5 card p-8 flex flex-col justify-center">
            <div className="flex items-center gap-2 mb-5">
              <Sparkles className="h-4 w-4 text-accent-indigo" />
              <span className="text-sm font-heading font-bold text-navy">Interests</span>
            </div>
            <div className="space-y-3">
              {INTERESTS.map((it) => (
                <div key={it} className="flex items-center gap-3 text-sm text-navy-600 font-body">
                  <span className="h-1.5 w-1.5 rounded-full bg-accent-indigo" />{it}
                </div>
              ))}
            </div>
          </motion.div>

          <motion.div {...fade(0.3)} className="md:col-span-12 mt-4">
            <div className="flex items-center gap-2 mb-5">
              <Award className="h-4 w-4 text-accent-gold" />
              <span className="text-sm font-heading font-bold text-navy">Certifications</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {CERTS.map((c) => (
                <a key={c.name} href={c.href} target="_blank" rel="noreferrer"
                   className="group card p-5 flex flex-col items-center text-center gap-3">
                  <div className="h-16 w-16 rounded-xl overflow-hidden bg-surface-100 flex items-center justify-center p-1.5">
                    <img src={c.logo} alt={c.name} className="h-full w-full object-contain group-hover:scale-105 transition-transform duration-300" />
                  </div>
                  <div>
                    <div className="text-sm font-heading font-bold text-navy group-hover:text-accent-indigo transition-colors">{c.name}</div>
                    <div className="text-[10px] text-navy-400 mt-0.5 leading-tight font-body">{c.full}</div>
                  </div>
                </a>
              ))}
            </div>
